...
def check_services():
    ...
    nfo("pidrive_display.service: dauerhaft deaktiviert (TFT entfernt v0.11.92)")
...
def check_display_env():
    ...
    nfo("TFT-Display + GPIO dauerhaft entfernt v0.11.92")
...
def check_vtcon():
    ...
    if not __import__("os").path.isdir("/sys/class/vtconsole"):
        ...
        return
    # fbcon=nodeconfig: nicht mehr relevant (TFT-Display entfernt v0.11.92)
...
def summary():
    ...
    ctx_checks = [
        ("pidrive_core",    "root"),
        ("pidrive_web",     "pidrive"),  # Futro: pidrive; Pi: pi
        ("pidrive_avrcp",   "root"),
    ]
    ...
    for check_path in ["/var/run/pulse", "/var/run/pulse/native", "/tmp/pidrive_cmd"]:
        if _ctx_os.path.exists(check_path):
            ...
            if check_path in ["/var/run/pulse", "/var/run/pulse/native"]:
                ...
                nfo(f"{check_path}: nicht vorhanden (kein Trigger gesetzt)")
