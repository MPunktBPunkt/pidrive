...
def prepare_audio_route(settings=None, source: str = "") -> dict:
    """v0.10.16: decide() + apply() — Rückgabe: Decision-Dict."""
...
def _pa_ok() -> bool:
    """
    v0.10.16: Socket-Datei zuerst prüfen (~0.1ms statt ~150ms systemctl subprocess).
...
def invalidate_sink_cache():
    """v0.10.16: Cache nach BT-Connect invalidieren."""
...
def decide_audio_route(settings=None, source: str = "") -> dict:
    """
    v0.10.16: Pure Policy — keine Side-Effects.
...
def apply_audio_route(decision: dict):
    """
    v0.10.16: Side-Effects aus decide_audio_route() anwenden.
...
def build_player_args(decision: dict, source: str = "") -> list:
    """v0.10.16: Baut mpv-Argumente aus Decision — keine Side-Effects."""
...
def get_mpv_args(settings=None, source: str = "") -> list:
    """v0.10.16: Wrapper — nutzt decide_audio_route() + apply_audio_route()."""
    d = decide_audio_route(settings=settings, source=source)
