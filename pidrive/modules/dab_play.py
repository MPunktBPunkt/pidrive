#!/usr/bin/env python3
"""dab_play.py — DAB+ Wiedergabe via welle-cli  v0.10.10"""
...
def play_station(station, S, settings=None):
    ...
    try:
        ...
        # ── v0.10.10: Saubere ALSA-Umgebung für welle-cli ────────────────────
        ...
        # ── v0.10.10 (korrigiert): ALSA→PulseAudio-Routing für welle-cli ────────
        ...
        for _k in ("PULSE_SINK", "PULSE_AUDIO"):
            _welle_env.pop(_k, None)
        # PULSE_SERVER bleibt → ALSA-PA-Plugin kann System-PA verbinden → Klinke
        ...
        # v0.10.10: Audio-Routing-Debug beim Start
        _pa_default = ""
        try:
        ...
        for _ in range(lock_wait_max):
            ...
            # v0.10.10: _sess_err_file statt globalem ERR_FILE (Session-Isolation)
...
def stop(S):
    ...
    if S.get("radio_type") == "DAB":
        S["radio_playing"] = False
        S["radio_station"] = ""
        # v0.10.10: Clear DLS/artist/track fields on stop to avoid stale display
        S["artist"] = ""
        S["track"] = ""
        S["dls_text"] = ""
        S["radio_name"] = ""
