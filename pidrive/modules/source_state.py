...
STATE = {
    "source_current":  "idle",
    "source_previous": "idle",   # v0.10.39: letzte Quelle vor aktuellem Wechsel
    "source_target":   "",
    "transition":      False,
    "owner":           "",
    "since":           0.0,
    ...
    "transition_count": 0,        # v0.10.39: Gesamtzahl Transitionen (für Diagnose)
    "stale_cleared":    0,        # v0.10.39: Zähler für automatisch abgeräumte Stale-Transitions
}
...
def _check_stale_transition() -> bool:
    """
    v0.10.39: Räumt hängende Transition auf ohne auf begin_transition() zu warten.
...
def force_end_transition(reason: str = "error"):
    """
    v0.10.39: Erzwingt Ende der Transition unabhängig vom aktuellen State.
...
def previous_source() -> str:
    """v0.10.39: Letzte Quelle vor dem aktuellen Wechsel."""
    with _LOCK:
...
def in_transition() -> bool:
    with _LOCK:
        # v0.10.39: Stale-Transitions nicht als aktiv melden
        if not STATE["transition"]:
            return False
        age = time.time() - STATE["since"]
        if age >= STALE_TIMEOUT_S:
