...
# -*- coding: utf-8 -*-
"""
avrcp_trigger.py — PiDrive AVRCP → File-Trigger Bridge  v0.10.50
...
# v0.10.50: Dedicated AVRCP-Log für Autotest
...
def _raw_log(line: str):
    """
    v0.10.50: Jeden D-Bus/bluetoothctl-Rohdaten-Eintrag in AVRCP_RAW_LOG schreiben.
...
def handle_avrcp(event: str, source: str = "unknown", raw_line: str = ""):
    """
    v0.10.50: Verarbeit ein AVRCP-Event mit vollständigem Verbose-Logging.
...
def monitor_bluetoothctl():
    """
    v0.10.50: bluetoothctl monitor mit vollständigem Raw-Logging.
...
def _parse_dbus_line(line: str) -> tuple[str | None, str]:
    """
    v0.10.50: D-Bus Zeile auf AVRCP-Events parsen.
...
def monitor_dbus():
    """
    v0.10.50: dbus-monitor mit vollständigem Raw-Logging und erweiterter Erkennung.
...
def main():
    log.setup("core")
    _sep("PiDrive AVRCP v0.10.50 gestartet")
    ...
    try:
        ...
        _raw_log(f"=== AVRCP v0.10.50 Start {time.strftime('%Y-%m-%d %H:%M:%S')} ===")
    except Exception:
        pass
