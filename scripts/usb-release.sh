#!/bin/bash
# USB-Freigabe für deferred ESP/RTL/QinHeng (authorized=0 → 1).
# Default (Boot): Quirks leeren, Genesys-Hub rebinden, authorize.
...
n=$(authorize_deferred)

udevadm settle --timeout=10 2>/dev/null || true
log "done (peripherals_seen=$n authorize_only=$AUTHORIZE_ONLY)"
