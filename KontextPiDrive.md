# PiDrive — Kontext & Projektdokumentation v0.9.30
...
### v0.10.43 — ROOT CAUSE GEFUNDEN: NaN/Infinity bricht gesamtes JavaScript
...
### v0.10.43 — JS-Fehlerdiagnose: globaler Error-Handler + console.log
...
### v0.10.43 — sendCmd Timing-Fix: Browser-Navigation funktioniert jetzt
...
### v0.10.43 — Menü-Fix komplett, Debug-Logging, Display-Guard
...
2. v0.10.43: `main_core.py` schrieb `menu.json` nur bei `needs_rebuild=True` — daher
...
### v0.10.43 — Navigation-Fix (Hauptfehler), DAB-Status, Display-Guard, Sudoers
...
### v0.10.43 — Root Cause Fix: PA-Setup ausserhalb Raspotify-Block, Diagnose-Kontext-Tests
...
### v0.10.43 — Code Review Fixes (P1/P2): BT, Dispatcher, Rebuild, Stale-State
...
### v0.10.43 — Debug-Tab Redesign, System-Diagnose, DAB Errfile, Webradio Metadaten
...
### v0.10.43 — Installer-Reihenfolge Fix, Log-Tab Fix, Diagnose-PA-Check
...
Fix v0.10.43: Bei Erstinstallation läuft Cleanup am Ende, gibt expliziten Hinweis
...
### v0.10.43 — Code Review Fixes, Installer Car-Only Cleanup, DAB ALSA-Direkt
...
```
✓ pidrive_core.service      v0.9.30 — BT Ghost-Connect, DAB/BT-Pause, PPM-Calibrate
✓ pidrive_display.service   v0.9.30, 20fps
...
## Bekannte Erwartete Warnungen (Expected Diagnostics)

Folgende Meldungen im Log sind **kein Fehler** und können ignoriert werden:

| Meldung | Ursache | Bedeutung |
|---|---|---|
| `fbcon not available` | TFT-Display nicht angesteckt | `pidrive_display.service` optional — Core läuft weiter |
| `DAB: no_lock` | Schlechter DAB-Empfang (Innenraum) | welle-cli läuft, kein stabiler Lock — im Auto mit Antenne OK |
| `bt_state=failed` beim Boot | Bluetooth-Geräte ausgeschaltet | BT-Watcher wartet — nach Pairing/Einschalten automatisch |
| `MPRIS2 Start: Unable to autolaunch` | Kein X11/Display für D-Bus | Systemd-Dienst ohne Desktop — fix: SystemBus wird genutzt |
| `Socket nicht gefunden: /tmp/pidrive_mpv.sock` | mpv noch nicht bereit | Metadaten folgen mit etwas Verzögerung |
| `RTL-SDR: staler Lock … gefunden` | Pi-Neustart mit hängendem Prozess | Wird automatisch bereinigt |
