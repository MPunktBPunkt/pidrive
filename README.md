...
[![Version](https://img.shields.io/badge/version-0.9.30-orange.svg)](https://github.com/MPunktBPunkt/pidrive/blob/main/pidrive/VERSION)
...
BT Ghost-Connect: rc=1 fälschlich als Erfolg gewertet. BT-Reconnect
pausiert während DAB läuft. PPM-Kalibrieren Endpoint hinzugefügt.

### v0.9.29 (2026-04-27)

install.sh: git stash für robustes settings.json-Update. Volume-Parse-Fix (pactl Format).
squelch/dab_channels Key-Alignment. FM FastScan: Multi-Confirm, bestätigt vs. schwach.
...
Boot-Resume: DAB/FM setzen jetzt `last_source` und benutzen `save_settings()`.
Nach ROCK FM → Reboot: `Boot-Resume: DAB → ROCK FM` statt Webradio.

### v0.9.26 (2026-04-27)

15 Fixes: WebUI PulseAudio/Volume/RTL-SDR/Sink-Inputs, BT Boot-State,
DAB DLS live (artist/track), FM RDS-Hook vorbereitet, audio_route Klinke beim Boot.

### v0.9.25 (2026-04-27)

BT Auto-Reconnect: Exponential Backoff (12→300s), Stop nach 20min.
Volume-Korrektur: settings.json > 100% → 90%. WebUI zeigt Klinke/HDMI/BT.
...
**DAB auf BT:** PULSE_ENV aus welle-cli entfernt — PULSE_SERVER/PULSE_SINK
verhinderten RTL2838 OFDM-Sync. Scanner: `--ao=alsa` statt `--ao=pulse`.

### v0.9.21 (2026-04-26)

**DAB endgültig:** `welle-cli -p "NAME"` spielt direkt via ALSA/PulseAudio —
kein HTTP-Modus, kein mpv, kein Warten. BT-Scan jetzt mit echtem `scan on`/`scan off`.
...
**DAB Ton endgültig:** welle-cli gibt raw PCM aus (kein Header) → mpv konnte Format
nicht erkennen. Fix: welle-cli als HTTP-Server, mpv auf `http://localhost:7981/mp3/<sid>` —
identische Architektur zu Webradio.

### v0.9.16 (2026-04-25)

**Drei Bugfixes:** DLS-Poller `NameError: SCAN_PORT` behoben (Closure-Scope),
Versions-Banner in `main_core.py` dynamisch, `diagnose.py` zeigt korrekte Version.
