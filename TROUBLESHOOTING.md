...
**Stand v0.11.83 · Plattform: Debian 13 (x86) / Raspberry Pi OS**
...
Ab v0.11.83 bestätigt der Agent `Request confirmation` automatisch.  
...
Ab v0.11.83 gefixt (bufsize: 1→4096). Falls noch hoch:
```bash
...
In `mpris2.py` ist das ab v0.11.83 korrekt — beim Import, nicht in `start_mpris2()`.
