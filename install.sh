#!/bin/bash
# ============================================================
# PiDrive Install Script v0.8.25
...
# v0.8.25: RPi.GPIO für Display-Tasten (Key1/Key2/Key3)
...
if [ -f /etc/raspotify/conf ]; then
    ...
    # v0.8.25: Zielarchitektur Option B — Spotify über zentralen PulseAudio-Pfad
  ...
  # v0.8.25: Pi 3B Klinken-Ausgang physisch aktivieren
...
if ! (cd "$INSTALL_DIR/pidrive" && python3 -c "import main_core" 2>/dev/null); then
  ...
  # v0.8.25: WebUI Import-Smoke-Test — verhindert stille Strukturfehler wie v0.8.12
...
# ── Optionaler Car-Only Cleanup (v0.8.25) ─────────────────────────────────────
